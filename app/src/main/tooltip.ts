...
// 자신의 첫 번째 창(windows[0], 보통 session_5h) 리셋까지 남은 시간을 'HhMm'으로 보여준다.
...
export function formatTooltip(state: AppState, now: number = Date.now()): string {
  ...
  const segments = present.map((provider, i) => {
    ...
    const pct = Math.round(state.limits[provider]!.windows[0].usedPercent)
    const isLast = i === present.length - 1
    const tail = isLast
      ? `오늘 ${formatMoney(totalCostUsd)}`
      : formatDuration(state.limits[provider]!.windows[0].resetsAt, now)
