...
pub fn record_snapshots(
    ...
    let tx = conn.transaction()?;
    {
        ...
        for w in windows {
            ...
            // resets_at은 f64 바인딩이지만 정수값이면 SQLite INTEGER affinity가 INTEGER로 저장 —
            // v1과 동일한 저장 표현.
            if v2 {
...
/// 해당 계정의 창별 최신 스냅샷 재구성. 행이 없으면 None.
...
pub fn latest_account_snapshot(
    ...
    provider: &str,
    account_id: &str,
) -> rusqlite::Result<Option<AccountSnapshot>> {
    ...
    let windows = kept
        ...
        .collect();
    Ok(Some(AccountSnapshot { windows, fetched_at }))
}
...
mod tests {
    ...
    fn record_dedups_unchanged_and_records_changed() {
        ...
        let w = |p: f64| vec![RateWindow { kind: "session_5h".into(), used_percent: p, resets_at: 999.0 }];
    ...
    fn record_works_on_v1_schema_without_account_column() {
        ...
        let w = vec![RateWindow { kind: "weekly".into(), used_percent: 5.0, resets_at: 999.0 }];
    ...
    fn latest_account_snapshot_picks_max_ts_and_orders_windows() {
        ...
        assert_eq!(snap.fetched_at, 2000.0);
    ...
    fn latest_account_snapshot_reads_real_typed_columns() {
        ...
        .unwrap();
        let snap = latest_account_snapshot(&conn, "claude", "a").unwrap().unwrap();
        assert_eq!(snap.fetched_at, 2000.5);
        assert_eq!(snap.windows[0].kind, "session_5h");
        assert_eq!(snap.windows[0].resets_at, 999.5);
    ...
    fn record_snapshots_tolerates_real_typed_prev_resets_at() {
        ...
        .unwrap();
        let w = vec![RateWindow { kind: "session_5h".into(), used_percent: 10.0, resets_at: 999.0 }];
