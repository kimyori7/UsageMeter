...
class SettingsDialog(ctk.CTkToplevel):
    def __init__(self, master, config: Config, on_save: Callable[[Config], None]) -> None:
        ...
        self.var_live_secs = ctk.StringVar(value=str(config.live_refresh_seconds))
        self.var_other_secs = ctk.StringVar(value=str(config.other_refresh_seconds))
    ...
    def _build(self) -> None:
        ...
        ctk.CTkOptionMenu(row1, values=["5", "10", "30", "60"],
                          variable=self.var_live_secs).pack(side="left", padx=8)
        row2 = ctk.CTkFrame(self, fg_color="transparent")
        row2.pack(fill="x", **pad)
        ctk.CTkLabel(row2, text="그 외:").pack(side="left")
        ctk.CTkOptionMenu(row2, values=["30", "60", "300"],
                          variable=self.var_other_secs).pack(side="left", padx=8)
    ...
    def _save(self) -> None:
        ...
        self.config_ref.live_refresh_seconds = int(self.var_live_secs.get())
        self.config_ref.other_refresh_seconds = int(self.var_other_secs.get())
